...
var currentUserId;
(function(){
  const dbRefObject = firebase.database().ref().child('details').orderByChild("currState").equalTo("pending").limitToFirst(11);
  const emptyNewsLetter = document.getElementById('empty_newsLetter');

  dbRefObject.on('child_added', snap => {
    if (snap.val().userID != currentUserId) {
      numBasic += 1;
      if (numBasic < 11) {
...
function createCard(snapValue, onMore){
  ...
  switch (snapValue.val().currState) {
    ...
    case 'pending':
      imgIndicator.setAttribute('src','source/ico/pending_new.png');
      divMore.style.backgroundColor = '#E67E22';
      aLink.innerText = 'Take action';
      aLink.setAttribute('href','pending_request.html#'+snapValue.key);
      break;
    default:
      break
  }
